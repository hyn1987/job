        ...
        <div class="row">
          <div class="col-sm-7 col-xs-12">
            <div class="info-line row">
              <div class="col-xs-9">{{ trans('common.timesheet') }} <strong>{{ formatMinuteInterval($total['mins']) }} hrs</strong>:
              </div>
            ...
            <div class="info-line row">
              ...
              <div class="col-xs-3 text-right">
                <strong>{{ $total['others'] < 0 ? '($'.formatCurrency(abs($total['others'])).')' : '$'.formatCurrency($total['others']).'' }}</strong>
              </div>
            </div>
            ...
            <div class="info-line row last">
              <div class="total-amount col-xs-12 text-right">
                <strong>{{ $total['others'] - $total['amount'] < 0 ? '($'.formatCurrency(abs($total['others']-$total['amount'])).')' : '$'.formatCurrency($total['others']-$total['amount']).'' }}</strong>
              </div>
            </div>
          ...
          <tbody>
            ...
            @else
              ...
              <tr class="status-{{ strtolower($d->status_string()) }}">
                <td>{{ $d->contractor_fullname }}</td>
                <td>{{ $d->status==Transaction::STATUS_AVAILABLE? date_format(date_create($d->done_at), "M d, Y"): date_format(date_create($d->created_at), "M d, Y") }}</td>
                <td>{{ $d->type_string() }}</td>
                <td>{!! $d->desc !!}</td>
